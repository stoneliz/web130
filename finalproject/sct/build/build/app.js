...
const getCampusArticles = `
    query getCampusArticles($where: ArticlesWhereArgs) {
        viewer {
...
const getCommunityArticles = `
    query getCommunityArticles($where: ArticlesWhereArgs) {
        viewer {
            allArticles(where: $where) {
                edges {
                    node {
    ...
    // Get category articles
const getOpinionArticles = `
    query getOpinionArticles($where: ArticlesWhereArgs) {
        viewer {
            allArticles(where: $where) {
                edges {
                    node {
...
const createArticle = `
mutation createArticleQuery($input: CreateArticlesInput!) {
...
let displayArticles = (articles) => {
    ...
    for (i = 0; i < numberToDisplay; i++) {
        let article = articles[i],
            $elem = $('#article'+ (i + 1));
        $elem.find('h2, h3').html(article.title);
...
if (js_page == 'home_page') {
    $.ajax({
        type: "POST",
        url: "https://us-west-2.api.scaphold.io/graphql/scct",
        data: JSON.stringify({
